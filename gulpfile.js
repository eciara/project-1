...
function createThumbnails(done) {
    src('./src/images/baner/**/*.{jpg,png}')
        .pipe(sharpResponsive({
            formats: [
            // jpg
            { width: 576, format: "jpeg", rename: { suffix: "x576" } },
            { width: 768, format: "jpeg", rename: { suffix: "x768" } },
            { width: 1200, format: "jpeg", rename: { suffix: "x1200" } },
            { width: 1920, format: "jpeg", rename: { suffix: "x9200" } },
            // webp
            { width: 576, format: "webp", rename: { suffix: "x576" } },
            { width: 768, format: "webp", rename: { suffix: "x768" } },
            { width: 1200, format: "webp", rename: { suffix: "x1200" } },
            { width: 1920, format: "webp", rename: { suffix: "x1920" } },
            ]
        }))
        .pipe(dest(config.dist.images)).on('error', function (e) { console.log(e) })
    done();
}
exports.resize = createThumbnails;
